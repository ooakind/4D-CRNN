...
for train, test in kfold.split(one_falx, one_y.argmax(1)):
    ...
    y_train = one_y[train]
    model.fit([x_train[:, 0], x_train[:, 1], x_train[:, 2], x_train[:, 3], x_train[:, 4], x_train[:, 5]], y_train, epochs=100, batch_size=64, verbose=0, callbacks=[wandb.keras.WandbCallback(labels=["Low", "High"])])
    # evaluate the model
