...
config={"epochs": 100, "batch_size": 64, "fold": 4}
wandb.init(project='emosync', config=config)
...
# Shuffle test
# shuffler = np.arange(one_falx.shape[0])
# np.random.shuffle(shuffler)
# one_falx = one_falx[shuffler]
# one_y = one_y[shuffler]

seed = 7
np.random.seed(seed)
kfold = StratifiedKFold(n_splits=wandb.config["fold"], shuffle=False)#shuffle=True, random_state=seed)#shuffle=False)#
...
for train, test in kfold.split(one_falx, one_y.argmax(1)):
    ...
    y_train = one_y[train]
    model.fit([x_train[:, 0], x_train[:, 1], x_train[:, 2], x_train[:, 3], x_train[:, 4], x_train[:, 5]], y_train, epochs=wandb.config.epochs, batch_size=wandb.config.batch_size, verbose=0, callbacks=[wandb.keras.WandbCallback()])
    # evaluate the model
    x_test = one_falx[test]
    y_test = one_y[test]
    scores = model.evaluate([x_test[:, 0], x_test[:, 1], x_test[:, 2], x_test[:, 3], x_test[:, 4], x_test[:, 5]], y_test, verbose=0, callbacks=[wandb.keras.WandbCallback()])

    wandb.log({"eval_accuracy": scores[1] * 100})
    print("%.2f%%" % (scores[1] * 100))
    all_acc.append(scores[1] * 100)

    pred = model.predict([x_test[:, 0], x_test[:, 1], x_test[:, 2], x_test[:, 3], x_test[:, 4], x_test[:, 5]])
    # aro_res['confusion_matrix'].append(confusion_matrix(aro_test.argmax(1), pred.argmax(1)))

    # val_predictions = model.predict(val_data)
    top_pred_ids = pred.argmax(axis=1)
    ground_truth_ids = y_test.argmax(axis=1)
